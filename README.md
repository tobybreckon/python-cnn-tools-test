...
N.B. Durham Students - ***if testing/using on the Durham University LDS (linux) system*** you need to first run ```tensorflow1.14.0.init```, ```pytorch1.3.1.init``` and ```opencv4.1.1.init``` in the (Linux, not the Python) command shell to add the relevant paths for these tools to the ```PYTHONPATH``` environment variable (and be sitting at a machine with a GPU in it!).
